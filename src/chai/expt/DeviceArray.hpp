...
namespace chai::expt
{
  ...
  class DeviceArray {
    public:
      ...
      explicit DeviceArray(const umpire::Allocator& allocator)
        : m_allocator{allocator}
      {
      }

      DeviceArray(std::size_t size, const umpire::Allocator& allocator = umpire::ResourceManager::getInstance().getAllocator("DEVICE"))
        : m_allocator{allocator}
      {
        resize(size);
      }
    ...
    private:
      T* m_data{nullptr};
      std::size_t m_size{0};
      umpire::Allocator m_allocator{umpire::ResourceManager::getInstance().getAllocator("DEVICE")};
  };  // class DeviceArray
}  // namespace chai::expt
