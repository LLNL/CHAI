...
//////////////////////////////////////////////////////////////////////////////
#ifndef CHAI_HOST_DEVICE_ARRAY_MANAGER_HPP
#define CHAI_HOST_DEVICE_ARRAY_MANAGER_HPP
...
{
  template <typename T>
  class HostDeviceArrayManager {
    public:
      HostDeviceArrayManager() = default;

      HostDeviceArrayManager(const umpire::Allocator& host_allocator,
      ...
      explicit HostDeviceArrayManager(std::size_t size)
      {
        resize(size);
      }

      HostDeviceArrayManager(std::size_t size,
      ...
      HostDeviceArrayManager(const HostDeviceArrayManager& other)
      ...
      HostDeviceArrayManager(HostDeviceArrayManager&& other)
      ...
      ~HostDeviceArrayManager()
      {
        m_host_allocator.deallocate(m_host_data);
        m_device_allocator.deallocate(m_device_data);
      }

      HostDeviceArrayManager& operator=(const HostDeviceArrayManager& other)
      {
        if (&other != this)
        {
          m_host_allocator.deallocate(m_host_data);
      ...
      HostDeviceArrayManager& operator=(HostDeviceArrayManager&& other)
      {
        if (&other != this)
        {
          m_host_allocator.deallocate(m_host_data);
...
#endif  // CHAI_HOST_DEVICE_ARRAY_MANAGER_HPP