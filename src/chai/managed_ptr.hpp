...
namespace chai {
   ...
   ///       method in place of a C-style array, wrap it in a call to chai::unpack
   ///       to extract the C-style arrays contained within the ManagedArray. This
   ///       will pass the extracted host C-style array to the host constructor and
   ///       the extracted device C-style array to the device constructor. If it is
   ///       desired that these host and device C-style arrays be kept in sync like
   ///       the normal behavior of ManagedArray, define a callback that maintains a
   ///       copy of the ManagedArray and upon the ACTION_MOVE event calls the copy
   ...
   ///       members. If a managed_ptr is passed to the make_managed method in place
   ///       of a raw pointer, wrap it in a call to chai::unpack to extract the raw
   ///       pointers contained within the managed_ptr. This will pass the extracted
   ///       host pointer to the host constructor and the extracted device pointer to
   ///       the device constructor. If it is desired that these host and device
   ///       pointers be kept in sync, define a callback that maintains a copy of the
   ///       managed_ptr and upon the ACTION_MOVE event call the copy constructor of
   ///       that managed_ptr.
   ...
   ///       on this synchronization. For example, the constructor of T might run out of
   ///       per-thread stack space on the GPU. If that happens, you can increase the
   ///       device limit of per-thread stack space.
   ///
   template <typename T>
   class managed_ptr {
      public:
